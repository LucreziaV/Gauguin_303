...
let container, stats, clock, gui, mixer;
...
function init() {
    ...
    // DICHIARAZIONE SUOLO
    const suoloGeometry = new THREE.PlaneGeometry(100,100);
    const suoloMaterial = new THREE.MeshPhongMaterial({
        color: "orange",
        shininess: 300,
    })

    const suolo = new THREE.Mesh(suoloGeometry, suoloMaterial );
    ...
    loader.load( '/scena_casa.glb', function ( gltf ) {
        model = gltf.scene;
    ...
    // GESTIONE ANIMAZIONI (loop o no)
    if ( gltf.animations && gltf.animations.length > 0 ) {
        mixer = new THREE.AnimationMixer( model );
        
        // Scrivere solo se ONCE ckè di default va in loop!!
        ...
        gltf.animations.forEach( ( clip ) => {
            ...
            const tipo_animazione = configurazioni[ clip.name ];

            if ( tipo_animazione === "once" ) {
                action.setLoop( THREE.LoopOnce ); 
                action.clampWhenFinished = true; // = Si ferma all'ultimo frame (non resetta)
